...
program Observables_single
  ...
  implicit none
  character(len=400) :: command_argument, image_dir
  ...
  if (len_trim(command_argument) == 0) then
    ...
    call message('Usage: '//trim(command_argument)//' <type> <parameters_file> <gal_id> <iz>'// &
                  ' <theta> <wavelength> <ignore_small_scale_field> [y] [z] [image_dir]',&
                  master_only=.true.)
...
!     call message('', master_only=.true., set_info=info)
  ...
  call IO_read_dataset_vector('Beq', props%igal, buffer, group='Output')
  props%Beq = buffer
