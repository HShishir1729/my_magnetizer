...
contains
  ...
  subroutine jobs_prepare(completed, incomplete)
    ...
    call read_SFR_offset() !added by CJ. Reads ratio between observed SFR 
                           ! and galform SFR at any redshift. For this, 
                        !we need to provide this offset at different z in a
                        !file which is specified in global_input_params.f90. 
    call read_v0_z() !added by CJ. Reads v0 as at all z outputs.  
                        !we need to provide z dependent v0 
                        !file which is specified in global_input_params.f90. 
