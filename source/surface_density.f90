...
contains
  ...
  function molecular_to_diffuse_ratio(rdisk, Sigma_g, Sigma_star) result(Rmol)
    ...
    Rmol = frac_mol * (Pnot/p_Rmol_P0)**(p_Rmol_alpha)
        ! frac_mol_without_dyn  is added by CJ. In R19 and J24, this is equal to 1. 
        ! Set frac_mol_without_dyn in global_input_parameters.f90  
    return
  ...
  function midplane_pressure_Elmegreen(rdisk, Sigma_g, Sigma_star, v_gas) result(P)
    ...
    if (present(v_gas)) then
      ...
      v_gas_SI= ISM_sound_speed_km_s(Mstars_disk+Mstars_bulge,SFR,Mgas_disk)*1d3
                                          !added by CJ
