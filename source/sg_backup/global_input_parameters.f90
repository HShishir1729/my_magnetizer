...
module global_input_parameters
  ...
  ! Compression options
  logical :: p_IO_compression =.false. !SG #.true. ! requires chunking!
  integer :: p_IO_compression_level = 6
  ! Number of galaxies after which output file is updated
  integer :: p_ncheckpoint =2000 !SG #5000
  ...
  double precision :: p_courant_v =0.15 !SG #0.09 !0.4 for Damp=T and nxphys=151
  double precision :: p_courant_eta =0.15 !SG #0.09 !0.3 for Damp=T and nxphys=151
  logical :: p_variable_timesteps = .true.
  integer :: p_nsteps_max =70000 !SG #20000
  ...
  integer :: p_random_seed =42 !SG #17
  ...
  integer :: p_nx_ref=81 !SG #101 !151
  ...
  double precision :: p_rmax_over_rdisk =2.7d0 !SG #2.75d0
  ...
  character(len=20) :: p_seed_choice ='floor' !SG #'random'
  ...
  double precision :: C_alp = 0.3d0 !SG #1.0d0  !CJ edit. C_alp=1.0d0
  ! FLOOR
  ...
  logical :: p_allow_positive_shears =.true. !SG #.false.
  ...
  double precision :: p_outflow_Vhot =320 !SG #425 ! eventually, this should be read
...
!!!!! 
      else if (cs_model == 18) then!SG:Median fitting of "Krumholz et al. 2018"
        if (SFR >= SFR_th) then 
