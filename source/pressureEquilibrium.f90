...
module pressureEquilibrium
  ...
  use surface_density, only: molecular_to_diffuse_ratio, exp_surface_density
  use input_params, only: r_bulge , r_halo, r_disk, Mhalo, Mstars_disk, &
                          Mstars_bulge, SFR, Mgas_disk, nfw_cs1
...
contains
  ...
  !elemental function computes_midplane_ISM_pressure_from_B_and_rho(B, rho) result(P)
                                   !Above line commented and below added by CJ 
  function computes_midplane_ISM_pressure_from_B_and_rho(B, rho) result(P)
   ...
   !                          * (p_ISM_sound_speed_km_s*1d5)**2 * rho
     !P = rho* ( 1d0 / p_ISM_gamma +  (1.+ 3.* fmag**2 ) * p_ISM_kappa/3.0 ) * &
     !  (1d5*ISM_sound_speed_km_s(Mstars_disk+Mstars_bulge,SFR,Mgas_disk) )**2 &
     !  + (B*1d-6)**2/4d0/pi
     P = rho* ( (p_ISM_sound_speed_km_s*1d5)**2.0 / p_ISM_gamma +  & 
             (1.+ 3.* frac_mag_P*fmag**2 ) * p_ISM_kappa/3.0 * &
       (1d5*ISM_sound_speed_km_s(Mstars_disk+Mstars_bulge,SFR,Mgas_disk) )**2.0 ) &
       + (B*1d-6)**2/4d0/pi
                                     !added by CJ based on Luke's Notes
                                     ! p_ISM_xi =3/2 *fBeq**2
    return
