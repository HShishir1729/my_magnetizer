...
module equ  !Contains the partial differential equations to be solved
  ...
  integer, save :: bp_unit = 91 !HS: Added to store the B_p profiles in between GALFORM snapshots from MAGNETIZER's profile evolution (Date: 29/01/26)
  logical, save :: bp_file_open = .false. !HS: Added to st0re the B_p profiles in between GALFORM snapshots from MAGNETIZER's profile evolution (Date:29/01/26)
  integer, save :: dt_unit = 92 !HS: Added to store the timesnaps in between GALFORM snapshots from MAGNETIZER's profile evolution (Date: 04/02/26)
  logical, save :: dt_file_open = .false. !HS: Added to store the timesnaps in between GALFORM snapshots from MAGNETIZER's profile evolution (Date: 04/02/26)
  integer, save :: bfloor_unit = 97
  integer, save :: afloor_unit = 96
  integer, save :: r_unit = 98
  logical, save :: bfloor_file_open = .false.
  logical, save :: afloor_file_open = .false.
  logical, save :: r_file_open = .false.

  character(len=256) :: bfloor_fname
  character(len=256) :: afloor_fname
  character(len=256) :: bp_fname
  character(len=256) :: r_fname
  ...
  contains
    subroutine pde(f,dfdt)
      ...
      ! --- Open Bfloor file ---
!      if (.not.bfloor_file_open) then
!         open(unit=bfloor_unit, &
!              file='/home/shishir/test/B_op_fiducial_single/Bfloor_profiles_6147fid.dat', &
!              status='replace', action='write')
!         bfloor_file_open = .true.
!      endif

      ! --- Open Afloor file ---
!      if (.not.afloor_file_open) then
!         open(unit=afloor_unit, &
!              file='/home/shishir/test/B_op_fiducial_single/Afloor_profiles_6147fid.dat', &
!              status='replace', action='write')
!         afloor_file_open = .true.
!      endif
...
!      write(bfloor_unit,'(*(E20.10))') Bfloor
!      write(afloor_unit,'(*(E20.10))') Afloor

      !HS: Open Bfloor file (once per galaxy) (Date: 03/03/26)
      if (write_bfloor) then
        if (.not. bfloor_file_open) then
           write(bfloor_fname,'(A,I0,A,A)') &
                '/home/shishir/test/B_op_fiducial_single/bfloor/Bfloor_profiles_', &
                current_gal_id, trim(output_suffix), '.dat'
           open(unit=bfloor_unit, file=trim(bfloor_fname), &
                status='replace', action='write')
           print *, 'Opening Bfloor file for galaxy ', current_gal_id
           bfloor_file_open = .true.
        endif
        write(bfloor_unit,*) Bfloor
      endif

      !HS: Open Afloor file (once per galaxy) (Date: 03/03/26)
      if (write_afloor) then
        if (.not. afloor_file_open) then
           write(afloor_fname,'(A,I0,A,A)') &
                '/home/shishir/test/B_op_fiducial_single/afloor/Afloor_profiles_', &
                current_gal_id, trim(output_suffix), '.dat'
           open(unit=afloor_unit, file=trim(afloor_fname), &
                status='replace', action='write')
           print *, 'Opening Afloor file for galaxy ', current_gal_id
           afloor_file_open = .true.
        endif
        write(afloor_unit,*) Afloor
      endif
      ...
      if (.not.Damp) then
        ...
        if (Dyn_quench) then
          ! print*,'B_p profile output',f(:,2)        !HS: added to obtain Bp
          ! profile for single galaxy (Date: 27/01/26)
!          if (.not.bp_file_open) then
!             open(unit=bp_unit, file='/home/shishir/test/B_op_fiducial_single/Bp_profiles_6147fid.dat', &
!                  status='replace', action='write')
!             bp_file_open = .true.
!          endif
!
!          write(bp_unit,*) f(:,2)
          ! HS: Here is where the changes to store Bp profiles ends (Date:27/01/26)


          !HS: Open Bp file (once per galaxy) (Date: 03/03/26)
          if (write_bp) then
            if (.not. bp_file_open) then
               write(bp_fname,'(A,I0,A,A)') &
                    '/home/shishir/test/B_op_fiducial_single/bp/Bp_profiles_', &
                    current_gal_id, trim(output_suffix), '.dat'
               open(unit=bp_unit, file=trim(bp_fname), &
                    status='replace', action='write')
               print *, 'Opening Bp file for galaxy ', current_gal_id
               bp_file_open = .true.
            endif
            write(bp_unit,*) f(:,2)
          endif

          !HS: Open r file (once per galaxy) (Date: 10/03/26)
          if (write_r) then
            if (.not. r_file_open) then
               write(r_fname,'(A,I0,A,A)') &
                    '/home/shishir/test/B_op_fiducial_single/r/r_profiles_', &
                    current_gal_id, trim(output_suffix), '.dat'
               open(unit=r_unit, file=trim(r_fname), &
                    status='replace', action='write')
               print *, 'Opening r file for galaxy ', current_gal_id
               r_file_open = .true.
            endif
            write(r_unit,*) r_kpc
          endif
