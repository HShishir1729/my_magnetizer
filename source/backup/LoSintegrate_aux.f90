...
!To avoid numerical convergence issues, intensities are computed multiple times by varying steps_xp until convergence is reached        
        ...
        i_run_vol_int = 0
        if (res2 < 1.e-8) then !If I is below a threshold, there is no point in finding I,Q,U using LoS integral 
          run_vol_int= .false.
...
!When calculated intensity is zero, then also error is set to zero. 
        ...
        ipfname =ipfname(:LEN(trim(ipfname))-10)//".txt"
        if (res2 > 1.e-9) then !If I is below a threshold, there is no point in finding I,Q,U using LoS integral 
           ...
           write(34,"(i5, 2f12.6, i8, 3f8.3, 4e14.5, 4f14.4)"),props%igal,props%z(iz), data%theta, &
           steps_los_orig,  data%wavelength, d_los, hfac, res2, res3, resq, resu, error_i, error_q, error_u, time_calc 
           close(34)
        end if
        res = res3
...
!Various quantities are then integrated along the LoS. 
    use input_constants
    use messages
    use global_input_parameters, only: number_of_redshifts, input_file_name
    use input_constants
    !CJ use FRB
    use interpolation
    type(Galaxy_Properties), intent(in) :: props
