...
!  integer :: iread=0 !defined this variable in global input parameters to be
!  used as a global variable as index of z array/
  ...
  double precision, protected :: r_l_kpc
  double precision, protected :: r_v_kpc
  ...
  integer, public ::  current_gal_id !HS: Changed to "public" while troubleshooting as I wanted to access a particular set of galaxies (Date: 03/03/26)
  ...
  subroutine set_input_params(gal_id, error)
    ...
    !added by CJ. Correcting offset between observed and galform SFR
    !This factor is 1 if correct_SFR_offset=0 in global_input_parameters.f90
    SFR  = SFR * SFR_offset_fac(iread)
