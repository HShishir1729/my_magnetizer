...
module dynamo
  ...
  contains
    ...
    function finish(cpu_time_start, gal_id, this_t) result(runtime)
      ...
      use equ, only: bfloor_unit, afloor_unit, bp_unit !HS: store troubleshooting .dat files once per galaxy (Date: 03/03/26)
      use equ, only: bfloor_file_open, afloor_file_open, bp_file_open !HS: store troubleshooting .dat files once per galaxy (Date: 03/03/26)
      ...
      ! ---- HS: Close floor/output files after each galaxy (Date: 03/03/26) ----
      if (bfloor_file_open) then
         close(bfloor_unit)
         bfloor_file_open = .false.
      endif

      if (afloor_file_open) then
         close(afloor_unit)
         afloor_file_open = .false.
      endif

      if (bp_file_open) then
         close(bp_unit)
         bp_file_open = .false.
      endif
