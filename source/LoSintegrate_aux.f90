...
module LoSintegrate_aux
  ...
  type Galaxy_Properties
    double precision, allocatable, dimension(:,:) :: Br, Bp, Bz, Beq
    double precision, allocatable, dimension(:,:) :: Rcyl, h, n
    double precision, allocatable, dimension(:) :: z, r_disk, SFR 
    double precision, allocatable, dimension(:) :: Mgas_disk, Mstars_disk, Mstars_bulge,Mhalo
  ...
  double precision :: Bavg, b0avg, Bmax, b0max,navg, havg, discvol
  contains
...
!creating  grids rval and pval with all r and phi points used in integration
    ...
    if (ldense_z) then
     ...
     if (lFRB) then
       ...
       !CJ  z_start = get_FRB_LoS_z_position(x_path_new, impact_y, z_path_new, &
          !                               props%h_FRB,                      &
          !                               props%r_disk(iz),                 &
          !                               props%Mgas_disk(iz),              &
          !                               props%Mstars_disk(iz),            &
...
!CJ    use surface_density, only: molecular_gas_surface_density
    use input_constants
   ...
   !CJ Sigma_m = molecular_gas_surface_density([R], r_disk, Mgas_disk, Mstars_disk)
    Sigma_m = 0.0
...
!Function that Observables/Observables_single calls to calculate various intensities, RM etc   
 function IntegrateImage(im_type, props,data,iz,number_of_calls,method,error,dust) result(res)
   use fgsl
   use global_input_parameters, only: number_of_redshifts, output_file_name, &
                         ISM_sound_speed_km_s, iread
...
!finer integration grid to improve accuracy
   ...
   double precision :: Mstars
!tolerence for stokes parameters in percent. Also threshold (I_pol/I).  
   double precision :: tol_i, tol_qu, pol_th, Isyn_th
   integer :: slash_indx
   character(len=120) :: ipfname
...
!    mthd = 'IntVol'
!    mthd = 'plain' ! 
   mthd = 'IntLoS' !This method comutes Intensity using Line of sight and volume integrals 
   ...
   select case (trim(mthd))
      ...
      case('IntVol') 
          ...
          res = res1
          !This method comute total Intensity by doing a volume integral
          !This method also compute total and polarized Intensities by doing an LoS integral
          !Total intensity by volume integral and LoS integral should match 
      case('IntLoS')
          !This will fix cosmic ray energy density by assuming equipartition between cosmic rays and magnetic fields 
        ...
        vol_int_dimn = '2D' !The volume integral will be a 2D integral
        call Total_Synchrotron_Intensity_volume_integral(props_glb, data_glb, iz_glb, vol_int_dimn, res2)
        print *, 'I(syn) from volume integral', props%igal, iz, res2
        ...
        ! All Parameters (including step_xp) affecting convergence of results  are fixed below.
        steps_xp       = 50  !This quantity should be even
        steps_los_orig = 70  !if this quantity is bigger than 90, numerical issues arise
        d_los          = 2.e-2 !Minimum grid seperation (resolution) along any given LoS 
        hfac           = 0.8  !vertical height from disc plane upto which LoS integrations are done is hfac * max(scale height)
                              !If hfac = 1 produces good accuracy for exponentially decaying magnetic fields and gas densities
                              ! Since integrand is exp(-4*h) under equiparition, hfac=0.8 is an accurate assumption 

!tol_i and tol_qu ->  percentage numerical convergence of integral for computing I,Q and U should be less than this 
!                    Numerical convergence is ensured by the method discribed above.
!pol_th -> If percentage of total Q or U is less than pol_th * I, then it won't computed.  
        tol_i  = 5.0
        tol_qu = 5.0
        pol_th = 0.01        
!If synchrotron intensity computed using volume integral is below Isyn_th (detection limit), then it won't be computed by LoS method. 
        Isyn_th = 1.e-7
...
!Polarized I (I_pol) is  computed by varying steps_xp until numerical convergence is reached        
        run_vol_int   = .false. !Intensities are computed for different steps_xp as long as run_vol_int = true
        i_run_vol_int = 0 ! How many times I is computed using LoS integral
        if (res2 < Isyn_th) then !If I_syn from volume integral is below a threshold, I,Q,U is NOT computed using LoS integral 
          run_vol_int= .false.
          print *, 'I(syn) less than threshold. Will not compute I, Q and U', props%igal, iz, res2
        end if
        time_calc = 0.0 !Total time for computing I using LoS integral is initialized
        run_vol_int= .false. !Will not compute I using LoS integral
        do while (run_vol_int) 
          steps_yp =  steps_xp !LoS are shot through an XY plane. The number of LoSs is decided by these variables. 
          call cpu_time(time1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!NEED TO CHANGE CONVERGENCE CRITERIA!!!!!!!!!!!!! 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          if (i_run_vol_int >=1 .and. res3 > 0.0) then  !i_run_vol_int>=1 ensures that the LoS integral is computed atleast twice
        ...
        ! Priting all outputs .  
          print "(i5, 2f10.4, 3f5.2, 4e12.4)",props%igal,props%z(iz), data%theta, &
                     data%wavelength, d_los, hfac, res2, res3, resq, resu
...
! Writing final (possibly) converged output to a file. If steps_xp >250, not sure if converged unless q and u are zero.  
        slash_indx = SCAN(output_file_name, '/', back=.true.)
        ipfname =output_file_name(1:slash_indx)//"IQU_"//output_file_name(slash_indx+1:)
        ipfname =ipfname(:LEN(trim(ipfname))-5)//".txt"
        
        Mstars = props%Mstars_disk(iz)+props%Mstars_bulge(iz)
        iread  = iz

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!! Need to write to file properly !!!!!!!!!!!
!! Errors when run over a parellel environement !!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        if (res2 > Isyn_th) then !If I is below a threshold, there is no point in finding I,Q,U using LoS integral 
           open(34,file=ipfname, FORM='FORMATTED', status='old', position="append")
           write(34,"(i5,    f12.6,     i5, f12.6,       2e14.5,     13e14.6)"),&
                props%igal, props%z(iz), iz, data%theta,  res2, res3, & 
                props%SFR(iz), props%Mhalo(iz), Mstars, props%Mgas_disk(iz), &
                ISM_sound_speed_km_s(Mstars,props%SFR(iz), props%Mgas_disk(iz)), props%r_disk(iz),  &
                discvol,havg,  Bmax, b0max, Bavg, b0avg, navg 
          ! write(34,"(  f12.6,     1e14.5,     4e14.6)"),&
          !      props%z(iz),  res2,&
          !      props%SFR(iz), Mstars, props%Mgas_disk(iz), discvol 
          close(34)
        end if
        res = res3
   end select
...
! This routine computes the total synchrotron intensity (not polarized intensity) by doing a 3D volume integral. 
...
! The below subroutine can perform 3D or 2D or 1D calculation of volume integral to compute total I_syn

! The code is tested for test galaxy with perfect disc/cylinder shape 
! and homogeneous constant field throught disc
! The face on and edge on values of I_syn can be computed analytically
! Analytic I_syn for test galaxy agrees with numerical value computed below.
subroutine Total_Synchrotron_Intensity_volume_integral(props,  data, iz, vol_int_dimn,  res1)
   ...
   double precision, dimension(steps_rint) :: rval, B2_rval, Br_muG, Beq_muG, Bp_muG, Bz_muG,  &
          n_cm3, ncr_cm3, h_kpc
   double precision, dimension(steps_rint) :: b0_muG, s2br_muG2 !small scale field 
   ...
   !temporary variable
   double precision :: Brmid, Bpmid, Bzmid,ncrmid, Ke_fac, hmid, int_syn1, &
                        int_syn2, rmid, pvalmid
   double precision :: b0mid, s2brmid !s2brmid
   real time1, time2, time3
   !double precision :: Bmax, Bavg, B2avg, havg
   double precision ::  B2avg, b02avg
...
! limits of r and phi variables 
   ...
   rlim(1) = props%Rcyl(iz,1)+1.e-10    !limits of r-variable- 0-rdisc
...
!creating  arrays (rval and pval) for r and phi with array size steps_rint 
  ...
  !Below step is to avoid floating point errors. Occassionally you may get  ray_kpc(steps_rint)> rlim(2)=rdisc
...
! Wavelength of observation 
   lambda_m = data%wavelength

!Midplane values of galaxy properties are loaded into arrays of size steps_rint (same as r-array size) 
   call interpolate( props%Rcyl(iz,:), props%Br(iz,:), rval, Br_muG) !Br in micro Gauss
   call interpolate( props%Rcyl(iz,:), props%Bp(iz,:), rval, Bp_muG) !Bphi in micro Gauss
   call interpolate( props%Rcyl(iz,:), props%Bz(iz,:), rval, Bz_muG) !Bz in micro Gauss
   call interpolate( props%Rcyl(iz,:), props%n(iz,:), rval, n_cm3) !number density of diffuse gas per cm3
   call interpolate( props%Rcyl(iz,:), props%h(iz,:), rval, h_kpc) !disc scale height as a function of r in kpc
   call interpolate( props%Rcyl(iz,:), props%Beq(iz,:), rval, Beq_muG) !equipartition field in micro Gauss
   ncr_cm3    = n_cm3  * ionisation_fraction !One of determining cosmic ray density 
   b0_muG     = fmag   * Beq_muG   ! rms value of small scale field = fmag * equipartition field strengh 
   s2br_muG2  = b0_muG * b0_muG / 3.0  !mean square value of br = b0^2/3 
   
   h_kpc  = 1*h_kpc !h_kpc will be later used as scale height of magnetic field. 
           !If scale height of magnetic field is > disc scale height then h_B = h_dic * factor


!The below three lines set large scale field to zero. So intensity has constribution only from random field
! must be commented when large scale field is not zero
!   Br_muG = 0.0 * Br_muG
!   Bp_muG = 0.0 * Bp_muG
!   Bz_muG = 0.0 * Bz_muG
!To set small scale field to zero
!b0_muG     = 0.0   * Beq_muG   !set equal to zero 


!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!Statistical analysis of magnetic field!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!To find the maximum and volume average value of magnetic field 
!Also finds volume average scale height and and diffuse number density
   ...
   do rint = 1, steps_rint-1, 1
         B2avg  = B2avg + 0.43*h_kpc(rint)* B2_rval(rint) * rval(rint) * ( rval(rint+1)-rval(rint) ) 
         b02avg = b02avg+ 0.43*h_kpc(rint)* b0_muG(rint)**2 * rval(rint) * ( rval(rint+1)-rval(rint) )
                                   !0.43 = (1-e^-2)/2
         navg  = navg  + 0.63* h_kpc(rint) * ncr_cm3(rint) * rval(rint) * ( rval(rint+1)-rval(rint) )
                                   !0.43 = 1-e^-1 
         havg  = havg  + h_kpc(rint) * rval(rint) * ( rval(rint+1)-rval(rint) )
   end do
   Bavg  = sqrt(B2avg / havg) !dividing by volume = integral h(r) r dr = r^2/2
   b0avg = sqrt(b02avg / havg) !dividing by volume = integral h(r) r dr = r^2/2
   navg  = navg/havg
   discvol = 6.283 * havg ! Here discvol = 2*pi * integral h r dr =  
   havg  = havg /  ( rlim(2)**2.0/2.0 )
  
   print "(A,  e10.4, A, e10.4, A, e10.4, A, e10.4, A, e10.4, A, e10.4)",  &
          'z=',props%z(iz), 'r=',rlim(2), 'B_max=',Bmax, 'b0max=',b0max, 'Bavg=',Bavg, 'b0avg=',B0avg    




!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!Inserts test model here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!The test model is a perfect disc galaxy with homogeneous field!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!Cosine of Sine of line of sight angle
   cost = COS(data%theta) 
   sint = SIN(data%theta)

!The spectral index of electrons
   s          = data%alpha
   splus1by4  = (s+1.0)/4.0
...
!factor to account for integration along z-direction
   ...
   if ( CR_B_equipartition ) then  
        factor_z = 1.0/4.0  ! h(r)*factor_z = int_0^infinity exp(-4z/h(r))   
   end if

   
!Below Computes I_syn by doing a single integral along r-direction
!Here phi and z integrals are done analytically
   if (vol_int_dimn == '1D') then
     ...
     do rint =1, steps_rint-1, 1
       ...
       else if (include_random_B == 1) then
         int_syn1 = int_syn1 +rmid*ncrmid*hmid*(Brmid**2.0+Bpmid**2.0+ 2.0*s2brmid) &
                                   * ( rval(rint+1)- rval(rint) )
         int_syn2 = int_syn2 + rmid * ncrmid * hmid *  (Bzmid**2.0+ s2brmid ) &
                                              * ( rval(rint+1) -rval(rint) )
       else 
         print *, 'Set include_random_B = 0 or 1'
         stop
       end if
     end do
...
! 2D numerical integration to compute I_syn on the galactic disc plane over  r and phi grid points defined above.
! seperate terms to integrate above and below the galactic plane
! Z-integral is done analytically
      ...
      do rint =1, steps_rint-1, 1
       ...
       !To or not to include  random field to calculate sqaure of total field
        if ( include_random_B == 0 ) then
          Bmag2   =  Brmid**2.0 + Bpmid**2.0 + Bzmid**2.0 !B^2
        else if ( include_random_B == 1 ) then
          Bmag2   =  Brmid**2.0 + Bpmid**2.0 + Bzmid**2.0 + 3.0*s2brmid !B^2
        else 
          print *, 'set inc_random_field = 0 or 1'   
          stop
        end if

        ! To calculate the cosmic ray component of the model for I_syn
        ! The cosmic rays can be included via Equipartition or vice versa
        if ( CR_B_equipartition ) then 
        ...
        !At a given r, this factor is incorporates various factors
        ...
        do pint =1, steps_pint-1, 1
           ...
           if ( include_random_B == 0 ) then
             ...
             int_syn_grid(rint, pint) = ( Brmid**2.0 + s2brmid)* (1 - (sint*cos(pvalmid) )**2.0 )  & 
                           + (Bpmid**2.0 + s2brmid) * (1 - (sint*sin(pvalmid) )**2.0 )  &
                           + (Bzmid**2.0 + s2brmid) * sint**2.0 
...
function emissivity_syn_constant(s) result(const)
 ...
 double precision :: c, m_e, e, kpc
                    !all the quantities below are in cgs units 
  c     =  2.99792458e10 !cm s^-1. 
  m_e   =  9.1094e-28 ! mass in gram
  ...
  const =  e**3.0/(m_e*c**2.0) * ( 3.0*e/( 12.5663706144 * m_e**3.0 * c**5.0) )**((s-1.0)/2.0) &
           * sqrt(3.0) / ( 12.5663706144 * (s+1.0) ) * gamma( (3.0*s-1.0)/12.0 ) * gamma( (3.0*s+19.0)/12.0 ) & 
           ...
           / 1.e-23   & !converting energy in cgs units to Janski 
           * kpc**3.0 & ! volume element in kpc**3.0
           * (1.e-6)**((s+1.0)/2.0) & !magnetic field in units of muG. Converting it into Gauss
           * 12.5663706144  !4 pi factor because emissivity is per unit steredian
...
function  Ke_equipart( Bsq,s) result(Ke)
 ...
 double precision :: ratio_np, E1, E2
    ...
    E1         = 1
    E2         = 100
    Ke  =  ratio_np * (0.7) * (1.e-12 * Bsq/25.1327412287)/( ((0.00160218)**(-0.7)) * (E1**(-0.7) - E2**(-0.7)) )!25.1327412287 = 8 pi
...
function ext_factor_luminosity(z1) result(ext_factor)
 ...
 do while ( z2 <= zfinal )
   ...
   var1 = var1 + 0.0002 * var2 
   z2   = z2 + 0.0001 
 end do
 var1     = (1.0+z1) * (2997.92/h) * var1 !here final var1 is luminosity distance. Varified by online calculator
 ext_factor  =  (1.0+z1) / ( 4*3.1415 * (var1*Mpc)**2.0 )  ! megaparsec in cm
    !The last (1.0+z1) factor accounts for K correction due to shrinking of observed frequency band. 
...
!LoS are defined by a plane that is perpenticular to LoS. This helps not miss any LoS passing through the galaxy. 
...
!Various quantities are then integrated along the LoS. 
    ...
    double precision, dimension(steps_rint) :: ray_kpc, Br_muG,  Bp_muG, Bz_muG, B2_rval, ncr_cm3, &
            n_cm3, ne_cm3, h_kpc
    double precision, dimension(steps_rint) :: b0_muG, Beq_muG, s2br_muG2 
    ...
    double precision :: s2brmid

    double precision, allocatable, dimension(:,:) :: Bx_xy_ay, By_xy_ay, Bz_xy_ay
    double precision, allocatable, dimension(:) :: x_ay, y_ay, xp_ay, yp_ay  
...
!On the r-phi grid points, magnetic field, scale height, and density are assigned from magnetizer output by interpolation
    ...
    call interpolate( props%Rcyl(iz,:), props%Beq(iz,:), ray_kpc, Beq_muG)
    b0_muG    = fmag * Beq_muG   !rms value of small scale field 
    s2br_muG2 = b0_muG*b0_muG/3.0  !mean square value of br = b0^2/3 
...
!The spectral index of synchrtron spectrum and internal polarization of source
    s          = data%alpha   !s ~ 3
    splus1by4  = (s+1.0)/4.0
    splus1by2  = 2.0*splus1by4  ! power of B_perp
    sminus1by2 = (s - 1.0)/2.0
...
!Simple toy model of a disk galaxy with constant values for magnetic field, scale height, and density.
...
!We assume galaxy is a disc with radius r_max and maximum height z_max. 
!limit of height of galactic disc. The z-integration is limited up to  hfac times maximum scale height. 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!LoS integral over all possible LoSs!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!Interpolating to find Br, Bphi, Bz, ncr, h etc on the above r and phi
             ...
             s2brmid =  s2br_muG2(rint)  + ratio_del_r * ( s2br_muG2(rint+1) - s2br_muG2(rint) ) 
...
!Total B_perp, B_parellel, B2 obtained by adding small scale B_perp,B_parellel, B2 
             if (include_random_B == 1) then
                 Bperp_los_sq = (Brmid**2.0 + s2brmid)* (1 - ( sint*cos(pval))**2.0 )  & 
                            + (Bpmid**2.0 + s2brmid) * (1 - ( sint*sin(pval))**2.0 )  &
                            + (Bzmid**2.0 + s2brmid) * sint**2.0  &
                            + sint**2.0 * sin(2.0*pval) * Brmid * Bpmid &
                            - sign(Bzmid,zval) * 2*sint*cost * ( Brmid*cos(pval)- Bpmid*sin(pval) )
                 Bmag2  = Bmag2 + 3.0*s2brmid
             end if
