...
contains
  subroutine write_output(gal_id, runtime)
    ...
    ! Writes log data
    call send_or_write_dataset('runtime', gal_id,                        &
                          [runtime],                                &
                          units='s',                                &
                          description='Running time of the galaxy', &
                          group='Log')

    call send_or_write_dataset('nsteps', gal_id,                         &
                          [dble(nsteps)],                           &
                          description='Number of timesteps',        &
                          group='Log')

    call send_or_write_dataset('status', gal_id,                         &
                          ts_status_code,                           &
                          description='Status codes.',              &
                          group='Log')

    call send_or_write_dataset('completed', gal_id,                      &
                          [1d0], group='Log')

    ! Outputs the data!
    call send_or_write_dataset('r', gal_id,                            &
                          ts_data%get('rkpc'),                    &
                          units='kpc',                            &
                          description='Radius')
...
!                           units='km/s', description='Radial velocity')
    ...
    if (p_extra_pressure_outputs) then
      if (ts_data%output('P')) &
        call send_or_write_dataset('P', gal_id, ts_data%get('P'), units='erg cm^-3', &
                              description='Pressure in the midplane.')
      if (ts_data%output('Pd')) &
        call send_or_write_dataset('Pd', gal_id, ts_data%get('Pd'), units='erg cm^-3',           &
                              description='Contribution of the gravity of the ' &
                              // 'diffuse gas to the pressure in the midplane')
      if (ts_data%output('Pm')) &
        call send_or_write_dataset('Pm', gal_id, ts_data%get('Pm'), units='erg cm^-3',           &
                              description='Contribution of the gravity of the ' &
                              // 'molecular gas to the pressure in the midplane')
      if (ts_data%output('Pstars')) &
        call send_or_write_dataset('Pstars', gal_id, ts_data%get('Pstars'), units='erg cm^-3',   &
                              description='Contribution of the gravity of the ' &
                              // 'stars to the pressure in the midplane')
      if (ts_data%output('Pbulge')) &
        call send_or_write_dataset('Pbulge', gal_id, ts_data%get('Pbulge'), units='erg cm^-3',   &
                              description='Contribution of the gravity of the ' &
                              // 'stellar bulge to the pressure in the midplane')
      if (ts_data%output('Pdm')) &
        call send_or_write_dataset('Pdm', gal_id, ts_data%get('Pdm'), units='erg cm^-3',            &
                              description='Contribution of the gravity of the '    &
                              // 'dark matter halo to the pressure in the midplane')
      if (ts_data%output('P2')) &
        call send_or_write_dataset('P2', gal_id, ts_data%get('P2'), units='erg cm^-3',  &
