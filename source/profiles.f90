...
contains
  logical function construct_profiles(B)
   ...
   ! use input_params! SG 
    ...
    v_kms = ISM_sound_speed_km_s(Mstars_disk+Mstars_bulge, SFR, Mgas_disk) &
                                  * p_ISM_kappa ! added by CJ
   ...
   ! sfr_new= SFR 
    ...
    if (r_disk < r_disk_min .or. Mgas_disk< Mgas_disk_min) then
            ! Sets the profiles to zero and returns!SG: It is alreday added in dynamo.f90. So don't need to repeat it here!
      n = 0.0
      l = 0.0
      h = 0.0
      Uz = 0.0
    ...
    Beq = fBeq*sqrt(4d0*pi*n) * v  !Formula for equiparition field strength !CJ Beq = sqrt(4d0*pi*n) * v 
